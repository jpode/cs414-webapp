class SinglePairOfLocations extends React.Component {
 ...
 WILL NEED TO BIND ALL FUNCTIONS (BY NAME) HERE FOLLOWING FORMAT */
  ...
  handleSubmit(event) {
    ...
    var tempNS = processInput(tempArray[0]);
    var tempEW = processInput(tempArray[1]);
    this.setState({ A_Coord_NS: tempNS });
    this.setState({ A_Coord_EW: tempEW });
    /*parsing B_Raw_Input */
    var tempArray = splitInput(this.state.B_Raw_Input);
    var tempNS = processInput(tempArray[0]);
    var tempEW = processInput(tempArray[1]);
    this.setState({ B_Coord_NS: tempNS });
    this.setState({ B_Coord_EW: tempEW });
    /*finding actual distance: */
    var tempDist = get_distance(
        this.state.A_Coord_NS,
        this.state.A_Coord_EW,
        this.state.B_Coord_NS,
        this.state.B_Coord_EW,
        "M"
      )
...
function splitInput(value) {
  console.log("input to SplitInput: " + value);
...
function processInput(value) {
  console.log("ProcessInput input: " + value);
